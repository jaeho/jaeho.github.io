...
What is Test Code?
-
꼬마 개발자 시절 나는 테스트에 대해 큰 관심이 없었습니다. 재미로 테스트 코드를 작성해본 경험은 있었지만 필요성을 느껴서 사용한 적은 없었죠. 지금에 와서 돌이켜 보면 그때의 나는 naive 했고 개발하던 프로젝트는 테스트를 필요로할만큼 크지도 않았습니다. (물론 프로젝트 규모가 작다고 테스트가 필요하지 않다는 의미는 아닙니다) 시간이 흘러 어린시절 보다 느린 반응 속도와 떨어지는 다중 처리 능력, 좁아터진 저장 공간과 무엇보다 개발 외에 신경써야 할 인생사가 많아진 지금에 나는 테스트에 다시금 관심을 갖게 되었습니다. 다소 떨어진 신체적 능력에 예전보다 커진 시스템을 다뤄야 하는 상황을 경험으로만 커버하기는 어려울것 같습니다.

구글에 "test code"를 검색해보면 결과가 8,060,000,000개, 범위를 조금 좁혀 "android test code"를 검색해 보면 1,190,000,000개가 검색 됩니다. 이처럼 많은 관심을 받고 있는 테스트 코드 우리는 어디까지 알고 어디까지 활용하고 있을까요?
